...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Merge multiple EgoExOR HDF5 datasets."
    )
    parser.add_argument(
        "--data_dir",
        type=str,
...
def detect_dataset_version(h5_path):
    ...
    with h5py.File(h5_path, "r") as f:
        ...
        if "data" in f:
            return "legacy"
    raise ValueError(
        "Unknown HDF5 schema: neither 'procedures' nor 'data' found at root"
    )
                    ...
                    for take_id in h5_file[take_path]:
                        ...
                        if frames_path in h5_file:
                            try:
                                ...
                                logger.warning(
                                    f"Error getting frame count for {frames_path}: {e}"
                                )
    else:  # hq
                ...
                for take_id in takes_grp:
                    ...
                    if frames_path in h5_file:
                        try:
                            ...
                            for frame_id in range(num_frames):
                                entries.append(
                                    (procedure, int(phase), int(take_id), frame_id)
                                )
                        except Exception as e:
                            logger.warning(
                                f"Error getting frame count for {frames_path}: {e}"
...
def _populate_split(h5_file, split_name, entries):
    ...
    h5_file.create_dataset(f"splits/{split_name}", data=split_data, compression="gzip")


def create_splits(
    h5_file, version, train_size=0.7, val_size=0.15, test_size=0.15, random_seed=42
):
    """
                ...
                if takes_path in in_file and takes_path in out_file:
                    for take_id in in_file[takes_path]:
                        ...
                        if take_id not in out_file[takes_path]:
                            ...
                            logger.warning(
                                f"Skipping {take_path} - already exists in output file"
                            )
...
def main():
    ...
    # Merge files
    try:
        merge_files(input_files, splits_file, output_file)
        return 0
    except Exception as e:
